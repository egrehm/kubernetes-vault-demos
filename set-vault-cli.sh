...
f_enable_proxy(){
ACTIVE_VAULT=$(kubectl -n vault get vault -o yaml | grep  -i active | cut -d : -f 2)
#echo " RUN: kubectl -n $NAMESPACE port-forward $ACTIVE_VAULT 8200 > /tmp/kube-proxy8200 2>&1 &"
PF_PID=$(ps aux | grep  "kubectl.*8200" | grep  -v grep | awk '{ print $2 }')
if [[ -z $PF_PID ]];then
...
kubectl get secrets -n vault vaultserver-vault-tls -o  jsonpath='{.data.tls\.crt}' | base64 -d > $CRT_FILE
echo "
You need to set env:
######
